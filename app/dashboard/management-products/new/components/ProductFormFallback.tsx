        ...
        <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in-50 duration-500">
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <Card className={`shadow-lg border-l-4 card-hover-effect card-border-glow transition-all duration-300 ${hasSuppliers
                    ...
                    <CardHeader className="pb-4">
                        <CardTitle className="flex items-center gap-2 text-lg">
                            <Icon name="ChefHat" className={`h-5 w-5 icon-enhanced ${hasSuppliers ? 'text-feature-suppliers' : 'text-feature-settings'
                                }`} />
                            <span>أنواع المطابخ</span>
                            <div className={`w-3 h-3 rounded-full animate-pulse ${hasSuppliers ? 'bg-feature-suppliers' : 'bg-feature-settings'
                                }`} />
                        </CardTitle>
                        <CardDescription>
                            {hasSuppliers
                                ? 'أنواع المطابخ متوفرة ويمكن استخدامها ✓'
                                : 'لا توجد أنواع مطابخ. يمكن إضافة منتج بدون نوع مطبخ ولكن يُفضل إضافة أنواع المطابخ'}
                        </CardDescription>
                    </CardHeader>
                    ...
                    {!hasSuppliers && (
                        <CardContent>
                            <Button asChild variant="outline" size="sm" className="btn-edit-outline btn-professional">
                                <Link href="/dashboard/management-Cuisine">
                                    <Icon name="ChefHat" className="w-4 h-4 me-2 icon-enhanced" />
                                    إدارة أنواع المطابخ
                                </Link>
                            </Button>
                        </CardContent>
